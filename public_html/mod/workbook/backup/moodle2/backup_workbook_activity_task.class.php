...
require_once($CFG->dirroot . '/mod/workbook/backup/moodle2/backup_workbook_stepslib.php'); // Because it exists (must)

/**
 * workbook backup task that provides all the settings and steps to perform one
...
class backup_workbook_activity_task extends backup_activity_task {
    ...
    static public function encode_content_links($content) {
        ...
        /// Link to workbook view by workbookid
        $search="/(".$base."\/mod\/workbook\/view.php\?d\=)([0-9]+)/";
        $content= preg_replace($search,'$@DATAVIEWBYD*$2@$', $content);

        /// Link to one "record" of the workbook
        $search="/(".$base."\/mod\/workbook\/view.php\?d\=)([0-9]+)\&(amp;)rid\=([0-9]+)/";
