...
class workbook_field_url extends workbook_field_base {
    var $type = 'url';
    ...
    function display_add_field($recordid=0) {
        ...
        if (!empty($this->field->param1) and empty($this->field->param2)) {
            ...
            $str .= get_string('url','workbook').':</td><td><input type="text" name="field_'.$this->field->id.'_0" id="'.$fieldid.'" value="'.$url.'" size="60" /></td></tr>';
            $str .= '<tr><td align="right">'.get_string('text','workbook').':</td><td><input type="text" name="field_'.$this->field->id.'_1" id="field_'.$this->field->id.'_1" value="'.s($text).'" size="60" /></td></tr>';
            $str .= '</table>';
        } else {
            // Just the URL field
        ...
        $module = array('name'=>'workbook_urlpicker', 'fullpath'=>'/mod/workbook/workbook.js', 'requires'=>array('core_filepicker'));
        $PAGE->requires->js_init_call('M.workbook_urlpicker.init', array($options), true, $module);
